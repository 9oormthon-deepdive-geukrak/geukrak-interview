...
1. 암묵적 결합
   - 전역 변수를 선언한 의도는 어디서든 참조하고 할당할 수 있는 변수를 사용하겠다는 것
   - 모든 코드가 전역 변수를 참조하고 변경할 수 있는 암묵적 결합을 허용하는 것
   - 유효 범위가 크면 클수록 코드의 가독성이 나빠지고 의도치 않게 상태가 변경될 수 있는 위험성이 높아진다.
2. 긴 생명 주기
   - 메모리 리소스의 오랜 기간 소비
...
- 생ㅓㅇ자 함수에 의해 생성된 객체를 인스턴스라고 한다.
- `String`, `Number`, `Boolean`, `Function`, `Array`, `Date`, `RegExp`, `Promise`등을 제공
...
const circle1 = new Circle(5);
console.log(circle1); // Circle { radius: 5, getDiameter: [Function: getDiameter] }
```
...
### 자바스크립트에서 함수가 일급 객체라면, 일급 객체로 뭘할 수 있나요?

- 값과 동일하게 취급할 수 있다.
  - 값을 사용할 수 있는 곳(변수의 할당문, 객체의 프로퍼티 값, 배열의 요소, 함수 호출의 인수, 함수 반환문)이라면 어디서든지 리터럴로 정의할 수 있으며 런타임에 함수 객체로 평가된다.
- 함수의 매개변수에 전달할 수 있다.
- 함수의 반환값으로 사용할 수 있다.
...
- 변수 사용을 억제하거나 생명주기를 최소화해서 상태 변경을 피해 오류를 최소화 하는 것을 목표

&nbsp;

### 꼬리 질문) 순수 함수가 뭔가요? 일반 함수와는 어떤 차이가 있죠?

- 외부 상태에 의존하지 않고 변경하지도 않는, 즉 부수 효과가 없는 함수
...
- 캡슐화
  - 관련있는 멤버 변수와 메서드를 클래스와 같은 하나의 틀 안에 담고 외부에 공개될 필요가 없는 정보는 숨기는 것
  - 정보 은닉
    - 자바스크립트에서는 `public`, `private` 등의 키워드를 제공하지 않는다.
    - scope와 `this` 바인딩을 활용해 정보를 은닉한다.
- 다형성
  - 특정 기능을 선언 부분과 구현 부분으로 분리한 후 구현 부분을 다양한 방법으로 만들어 선택해서 사용할 수 있게 하는 기능
  - 자바스크립트는 언어적 특성을 이용해 다형성스럽게 코드를 작성한다.

&nbsp;

### 자바스크립트는 객체지향 프로그래밍 언어인가요?

- 자바스크립트 설계자인 브랜든 아이크는 객체 지향 프로그래밍에 회의적인 시각을 가지고 만들었다.
- 자바스크립트는 객체 지향 프로그래밍과 완전히 들어맞는 언어는 아니다.
- 자바스크립트는 객체지향의 패러다임은 계승하면서도 단순하고 간단한 언어를 추구하다보니 Java의 문법을 쓰지만 함수형을 기반으로 하는 class-free 프로토타입 기반의 OOP를 도입했다.
  - 객체지향 맛 언어
- 코드의 재사용을 높이고, 간결한 문법의 장점은 취하고, 복잡한 구조를 만들지 않도록 지양하고, 함수형 프로그래밍이 가지고 있는 장점과 결합하는 것이 현대 자바스크립트에서 객체지향 프로그래밍을 잘한다라고 볼 수 있는 것 같다.
...
- `with` 문의 사용
- 일반 함수의 `this`
  - 함수를 일반 함수로서 호출하면 `this`에 `undefined`가 바인딩
- `arguments` 객체
...
- `Object`, `String`, `Array`, `Function`, `Date`, `RegExp`, `Error`, `Math`, `JSON` 등
...
- 함수 호출 방식에 의해 동적으로 `this` 바인딩이 결정된다.
...
- 일반 함수 호출: `this`에는 전역 객체가 바인딩
- 메서드 호출: 메서드 이름 앞의 마침표(`.`) 연산자 앞에 기술한 객체가 바인딩
...
- 실행 컨텍스트는 식별자를 등록하고 관리하는 스코프와 코드 실행 순서 관리를 구현한 내부 매커니즘
