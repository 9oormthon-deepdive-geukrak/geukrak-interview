...
- 콜백을 사용할 수 있다.
  - 함수가 다른 함수에 인자로 전달되어 특정 시점에 호출됨됨
...
- 자바스크립트에서는 객체가 다른 객체를 상속받을 때 클래스 대신 프로토타입을 사용
- 모든 객체는 숨겨진 [[Prototype]] 속성을 통해 다른 객체에 접근할 수 있음
- 새로운 객체를 생성할 때 기존 객체를 프로토타입으로 설정하여 상속 계층을 구성
